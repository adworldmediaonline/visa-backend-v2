...
const kenyaPaymentController = {
  ...
  createPaymentOrder: expressAsyncHandler(async (req, res) => {
    try {
      ...
      const application = await KenyaVisaApplication.findById(
        formId
...
const sendConfirmationEmail = async (email, id, domainUrl) => {
  try {
    const mailOptions = {
      to: email,
      subject: 'Kenya eVisa Payment Confirmation',
      html: `<p>Dear Applicant,</p>
             <p>Thank you for your payment. Your Kenya eVisa application has been received and is now being processed.</p>
             ...
             <p>Best regards,<br>Kenya eVisa Team</p>`,
    };
...
export default kenyaPaymentController;
