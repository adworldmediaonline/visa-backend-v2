  ...
  createEthiopiaVisaDetails: async (req, res) => {
    try {
      ...
      const ethiopiaVisaApplicationResult =
        await ethiopiaVisaApplication.save();
      ...
      console.error(
        'Error creating Ethiopia visa application and details:',
        error
      );
  ...
  updateEthiopiaVisaDetails: async (req, res) => {
    try {
      ...
      if (updateData.visaType || updateData.visaValidity) {
        const visaType =
          updateData.visaType ||
          (await EthiopiaVisaDetails.findOne({ formId })).visaType;
        const visaValidity =
          updateData.visaValidity ||
          (await EthiopiaVisaDetails.findOne({ formId })).visaValidity;
